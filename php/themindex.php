   ...
   require("connect.php");
        ...
        if ($uploadOk == 0) {
          ...
          // if everything is ok, try to upload file
            ...
            if (move_uploaded_file($_FILES["upload_file"]["tmp_name"], $target_file)) {
                ...
                $sql_insert = "insert into tbl_homeindex(cate_name,intro_img,cate_minute) values('".$cate_name."','".$target_file."','".$cate_minute."')";
    ...
    if(isset($_POST["delete_check"])){
        ...
        foreach($new_ids as $c){
            $sql_delete = 'DELETE FROM tbl_homeindex WHERE cate_id = '.$c;
            if(mysqli_query($conn,$sql_delete)){
                //xoa thanh cong
            }

        }
        header("location:themindex.php");
    }
    //xoa
    if(isset($_GET["task"]) && $_GET["task"]=="delete"){
        $id = $_GET["id"];
        $sql_delete = "delete from tbl_homeindex where cate_id = " . $id;
        if (mysqli_query($conn, $sql_delete)) {
            //echo "New record created successfully";
            header("location:themindex.php");
        } else {
    ...
    <body style="background-color: #ffffcc;">
        <div class="container">
            ...
            <?php
                ...
                $result = mysqli_query($conn, 'select count(cate_id) as total from tbl_homeindex');
                    ...
                    <table class="table table-stripped">
                        ...
                        <?php
                            ...
                            if(isset($_POST["txt_search"])){
                                ...
                                $sql = "select * from tbl_homeindex where cate_name like '%".$cate_name."%'";
                            }
                            else
                            $sql = "select * from tbl_homeindex ORDER BY cate_id DESC LIMIT ".$start.",". $limit ;
                            ...
                            if(mysqli_num_rows($result)>0){
                                while($row = mysqli_fetch_assoc($result)){
                                    ...
                                    echo "<td>";
                                        echo "<a class='btn btn-warning' href='?task=update&id=".$row["cate_id"]."'>Sửa</a>";
                                        echo "<a class='btn btn-danger' href='themindex.php?task=delete&id=".$row["cate_id"]."'>Xóa</a>";
                                    echo "</td>";
                                    echo "<td>";
                ...
                <div class="pagination">
                    <?php 
                        ...
                        if ($current_page > 1 && $total_page > 1){
                            echo '<a href="themindex.php?page='.($current_page-1).'">Truoc</a> | ';
                        }
                        ...
                        for ($i = 1; $i <= $total_page; $i++){
                            ...
                            if ($i == $current_page){
                                ...
                                echo '<a href="themindex.php?page='.$i.'">'.$i.'</a> | ';
                            }
                        }
                        
                        // nếu current_page < $total_page và total_page > 1 mới hiển thị nút prev
                        if ($current_page < $total_page && $total_page > 1){
                            echo '<a href="themindex.php?page='.($current_page+1).'">Sau</a> | ';
                        }
                    ?>
                </div>
            </div>
